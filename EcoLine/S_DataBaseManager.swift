...
//標準のリストに表示するためのselect文
    static func selectDataBase() -> [String]{
        do{
            ...
            if sqlite3_open(dbPath, &dbPointer) == SQLITE_OK{
                ...
                //SQLインジェクション対策が必要　　　　　　　　　　　　　　　　  「？」はプレースホルダー
                let query = "SELECT DATA_NAME FROM EcoLineDB WHERE LIMIT_TIME > ?;"
                ...
                if sqlite3_prepare_v2(dbPointer, query, -1, &statementCommand, nil) == SQLITE_OK{
                    ...
                    //クエリを実行する　　sqlite3_stepこのメソッドはDBの挿入、更新、削除の処理ができる
                    //エラーが出て実行できなかったらエラ〜チェックする
                    while sqlite3_step(statementCommand) == SQLITE_ROW{
                        let data_Name = String(cString: sqlite3_column_text(statementCommand, 0))
                        dataNameArray.append(data_Name)
                    }
    ...
    static func limitSelectDataBase() -> [String]{
        do{
